		...
		try {
			...
			if (error instanceof Error) {
				if (error.name === "MissingSchemaError") {
					throw new Error(
						"Event model not registered. Please ensure Event model is loaded.",
					);
				}
				// Re-throw the validation error we just created or other errors
				throw error;
			}
			throw new Error("Failed to validate event reference. Please try again.");
		}
	}
});
